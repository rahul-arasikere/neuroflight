...
serialPort_t* getUART4() {
	if(uart4Serial == NULL) {
		uart4Serial = openSerialPort(SERIAL_PORT_UART4, FUNCTION_BLACKBOX, NULL, NULL, 115200, MODE_RXTX, SERIAL_HW_FLOW_CTRL);
		if(millis() < 2000) // waiting 10 seconds so that the xbee connects since drone power on
			delay(2000 - millis());
