...
def live_ddpg(obs_queue, obs_space, act_space, hp: HyperParams=HyperParams(),actor_critic=core.mlp_actor_critic, logger_kwargs=dict(), save_freq=1, on_save=lambda *_:(), anchor_q=None):
    ...
    anchor_obs_replay = pickle.load( open( "obs_replay.p", "rb" ) )
    ...
    @tf.function
    def pi_update(obs1, obs2, anchor_obs):
        with tf.GradientTape() as tape:
            pi = pi_network(obs1)
            anchor_pi = pi_network(anchor_obs)
            pi2 = pi_network(obs2)
            q_pi = tf.reduce_mean(tf.squeeze(q_network(tf.concat([obs1, pi], axis=-1)), axis=-1))
            if anchor_q:
                anchor_q_pi = tf.reduce_mean(tf.squeeze(anchor_q(tf.concat([anchor_obs, anchor_pi], axis=-1)), axis=-1))
                q_c = tf.squeeze(obs_utils.p_mean(tf.stack([q_pi**0.5, anchor_q_pi**2.0]), 0.0))
            else:
                anchor_q_pi = 1.0
                q_c = q_pi
            ...
            action_c = obs_utils.p_mean(obs_utils.p_mean(1.0 - action_diffs, 1.0), 1.0)
            center_c = obs_utils.p_mean(obs_utils.p_mean(1.0 - tf.abs(pi+0.4)/1.5,1.0),1.0)
            # tf.print("q_c",q_c)
            reg_c = tf.squeeze(obs_utils.p_mean(tf.stack([action_c**1.5, center_c],axis=1), 0.0))
            # tf.print("action_c", action_c)
            # tf.print("center_c", center_c)
            # tf.print("r",reg_c)
            # tf.print("q",q_c)
            # # tf.print("")
            with_center_c = obs_utils.p_to_min(tf.stack([q_c,reg_c**0.5]))
        ...
        return pi_loss, avoid_extremes, q_pi, anchor_q_pi, reg_c
    ...
    while True:
        ...
        if t % hp.train_every == 0:
            ...
            for _ in range(hp.train_steps):
                ...
                anchor_obs_batch = anchor_obs_replay[np.random.randint(0, anchor_obs_replay.shape[0], size=hp.batch_size)]
                anchor_obs = tf.constant(anchor_obs_batch)
